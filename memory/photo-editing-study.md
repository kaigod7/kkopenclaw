...
### 💡 AI+手工混合精修工作流（进阶）
- **AI D&B 先跑一版**：Retouch4me或AI插件自动做光影塑造（已有七八分），再手动修正下颌线等过渡
- **磨皮强度控制**：别超70%，常用50-60%；100%放大永远觉得不够，但别人看的是整体
- **区分瑕疵与特征**：美人痣不动、卧蚕不磨（眼神好看的关键）
- **五官微调原则**：幅度≤5%，让对方成为最好版本的自己，不是变成另一个人

### 🎨 调色两种模式
- **AI预设**：快速出片，适合社交媒体/批量客片（FlowPix/Luminar Neo）
- **手动精细调色**：商业广告/高端写真，Camera Raw里HSL逐通道调+分离色调+曲线
...
### 🎨 PS调色核心命令详解
**色阶**：直方图+黑白灰吸管定义黑白场和灰场，校色基准
**曲线**：X轴输入/Y轴输出，S曲线增强对比，可铅笔手绘波形
**色相/饱和度**：吸管精准选取颜色范围，容差值控制精度
**色彩平衡**：阴影/中间调/高光分别调，冷暖色调修正
**可选颜色**：印刷级精准调特定色（青/品红/黄/黑）
**通道混合器**：制作细腻灰度图+做色调分离
**匹配颜色**：将A图的色调匹配到B图，合成必备
**替换颜色**：精准选择并替换任意颜色

### 💡 水下摄影修图思路
- 白平衡校正最关键（去除蓝绿色偏）
- HSL逐通道调色（红/橙/黄/绿/蓝各自调整）
- 去雾谨慎用，过度会破坏立体感

### 🔲 蒙版高级用法
- **亮度转不透明度**：编辑→将亮度转换为不透明度，分离高光/阴影分别建立蒙版
- **快速蒙版**：Ctrl+点击图层缩略图载入选区，配合滤镜做动感模糊/高斯模糊等创意效果
- **剪贴蒙版**：颜色/渐变填充图层剪贴到目标图层，实现非破坏性叠加效果

### 🎨 27种图层混合模式核心分类
**变暗组**（去浅色）：正片叠底/颜色加深/线性加深 → 做阴影
**变亮组**（去深色）：滤色/颜色减淡/线性减淡 → 做高光
**对比组**：柔光/强光/亮光/线性光/点光 → 增加对比质感
**色彩组**：色相/饱和度/颜色/明度 → 只影响色彩
**差值组**：差值/排除 → 创意反转效果

### 💡 混合模式实战
- **柔光**：50%灰以上提亮，以下压暗，柔化光影必备
- **正片叠底**：深色盖掉浅色，做阴影层
- **滤色**：浅色盖掉深色，做高光层
- **颜色减淡**：提亮中间调，比滤色更饱和
- **线性光**：D&B核心工具，结合中性灰使用

### 🔧 PS 2026 Remove Tool 核心用法
- 双击背景图层解锁，否则移除工具灰色不可用
- 两种模式：对象移除（Object Removal）/ 区域移除（Area Removal）
- 画笔比目标区域稍大，AI自动识别
- Generative AI on：AI生成填充；Auto：自动选择模式
- 不消耗 Generative Credits（免费）
- Sample All Layers：可从所有图层采样

### 🔤 3D文字旋转效果（PS 2026新功能）
- 文字工具 → 3D旋转对象 → 可调 spin/tilt/dolly 多视角
- 初始低分辨率预览，点击Done后用生成 Credits 升分辨率
- 适合海报/广告的立体文字效果

### 🎨 Harmonize（协调）功能 - 光影色调智能匹配
- 自动分析背景光照方向/强度/色温，自动修正主体色彩与光影
- 柔化抠图残留边缘，生成符合环境的反射和阴影
- 适用：产品植入场景/人物换背景/数字拼贴
- 操作：主体图层 → 协调 → 自动生成3种融合方案

### ⚡ AI锐化（Camera Raw）
- 智能区分不同区域（皮肤/发丝/岩石/天空）针对性锐化
- 动态边缘保护，防止白边/断层
- 噪点同步抑制，细节平衡滑块（清晰度vs自然度）
- 支持RAW非破坏性+批量处理动作

### 🏔️ 风光后期6课体系
1. 色彩对比和强化
2. 光感渲染（Lightbleeding）+ 色彩美学
3. 魔幻暗调（暗而不黑+朦胧感+光感过渡）
4. 质感和影调
5. 完整合成+调色实战（上）
6. 夜景+星空处理

### 📱 社交媒体多平台适配
- Instagram Story（1080x1920）：纵向生成式扩展
- Facebook帖子（1200x630）：横向裁剪+扩展
- 统一品牌Logo叠加确保全平台视觉一致
